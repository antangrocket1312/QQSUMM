...
def calculate_similarity(text1, text2):
    ...
    """
    return float(text1[1] @ text2[1])


def average_similarity_to_cluster(kp, kps_other):
    ...
    for kp_other in kps_other:
        ...
        total_similarity += [calculate_similarity(kp, kp_other)]

    return statistics.mean(total_similarity)


def deduplicate(sentences, embeddings, threshold):
    """ Group similar aspect terms of a business in a greedy fashion."""

    filtered = []
    for sent in zip(sentences, embeddings):
        find_merge = False

        similarity_to_other_clusters = []
        # Get best cluster
        for sents_cluster in filtered:
            #             average_cosine = average_similarity_to_cluster(sent, sents_cluster, sent_buss_df)
            average_cosine = average_similarity_to_cluster(sent, sents_cluster)
        ...
        if len(sorted_cluster_indices) > 0:
            ...
            if similarity_to_other_clusters[optimal_cluster_index] >= threshold:
                sents_other = filtered[optimal_cluster_index]
                sents_other.append(sent)
                find_merge = True

        if not find_merge:
            filtered.append([sent])

    return [[s[0] for s in cluster] for cluster in filtered]
