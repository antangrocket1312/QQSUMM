    ...
    def forward(
        ...
        if not self.is_decoder:
            try:
                ...
                print(e)
                lower_suitable = nearest_lower_divisor(input_ids.size()[1], self.config.n_context)
                input_ids = input_ids.view(input_ids.size(0) * lower_suitable, -1)
                attention_mask = attention_mask.view(attention_mask.size(0) * lower_suitable, -1)
